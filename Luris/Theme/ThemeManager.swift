...
extension View {
    ...
    func accentGlow(radius: CGFloat = 8) -> some View {
        self.shadow(color: Luris.accent.opacity(0.45), radius: radius, y: 2)
            .shadow(color: Luris.accentRose.opacity(0.25), radius: radius * 1.5, y: 0)
    }

    func accentCapsule() -> some View {
        self.background(Luris.accentGradient).clipShape(Capsule())
    }

    func accentRounded(radius: CGFloat = Luris.buttonRadius) -> some View {
        self.background(Luris.accentGradient)
            .clipShape(RoundedRectangle(cornerRadius: radius))
    }
}
            ...
            ZStack {
                ...
                LinearGradient(
                    stops: [
                        ...
                        .init(color: Luris.bg.opacity(0.55), location: 0.40),
                        .init(color: Luris.bg, location: 0.50),
                    ],
                    startPoint: .top,
                    endPoint: .bottom
                )
    ...
    func body(content: Content) -> some View {
        content.overlay(
            RoundedRectangle(cornerRadius: cornerRadius)
                .stroke(Luris.accentGradient, lineWidth: lineWidth)
        )
    }
}
