...
final class GenerationsViewModel: ObservableObject {
    ...
    func loadHistory() async {
        ...
        currentPage = 1
        do {
            let items = try await BabyUltraAPI.shared.getHistory(page: 1, limit: pageSize)
            history = items
    ...
    func loadMore() async {
        ...
        do {
            let items = try await BabyUltraAPI.shared.getHistory(page: nextPage, limit: pageSize)
            if items.isEmpty {
                hasMore = false
            } else {
    ...
    func deleteHistoryItem(id: Int) async {
        do {
            try await BabyUltraAPI.shared.deleteHistoryItem(id: id)
